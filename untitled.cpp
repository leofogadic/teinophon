...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("library_path4", "/libraries/physmodels.lib");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		...
		ui_interface->addHorizontalSlider("gain", &fHslider0, FAUSTFLOAT(0.0f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.00999999978f));
		ui_interface->addHorizontalSlider("pressure", &fHslider2, FAUSTFLOAT(0.5f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.00999999978f));
		ui_interface->addHorizontalSlider("velocity", &fHslider1, FAUSTFLOAT(0.0f), FAUSTFLOAT(0.0f), FAUSTFLOAT(1.0f), FAUSTFLOAT(0.00999999978f));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		float fSlow2 = (5.0f - (4.0f * float(fHslider2)));
		for (int i0 = 0; (i0 < count); i0 = (i0 + 1)) {
			iRec14[0] = 0;
			float fRec18 = (float(iRec14[1]) - (0.989264667f * ((0.600000024f * fRec19[2]) + (0.200000003f * (fRec19[1] + fRec19[3])))));
			fRec20[0] = fRec0[1];
			fRec21[(IOTA & 31)] = (-1.0f * (0.99880147f * ((0.800000012f * fRec20[2]) + (0.100000001f * (fRec20[1] + fRec20[3])))));
			float fTemp0 = fRec21[((IOTA - 27) & 31)];
			fRec22[0] = (fSlow1 + (fConst1 * fRec22[1]));
			float fTemp1 = (fRec22[0] - (fRec6[1] + fTemp0));
			float fTemp2 = (fTemp1 * std::min<float>(1.0f, (1.0f / mydsp_faustpower4_f((std::fabs((fSlow2 * fTemp1)) + 0.75f)))));
			fVec0[(IOTA & 31)] = (fTemp0 + fTemp2);
			fRec19[0] = fVec0[((IOTA - 17) & 31)];
			float fRec15 = fRec18;
			float fRec16 = fRec19[0];
			float fRec17 = fRec19[0];
			fRec10[(IOTA & 31)] = fRec15;
			float fRec11 = fRec10[((IOTA - 16) & 31)];
			float fRec12 = fRec16;
			float fRec13 = fRec17;
			fRec6[0] = fRec11;
			...
			float fRec9 = fRec13;
			fRec2[(IOTA & 31)] = fRec7;
			float fRec3 = fRec2[((IOTA - 26) & 31)];
			float fRec4 = fRec8;
			float fRec5 = fRec9;
			fRec0[0] = fRec3;
			float fRec1 = fRec5;
			float fTemp3 = (fSlow0 * fRec1);
			output0[i0] = FAUSTFLOAT(fTemp3);
			output1[i0] = FAUSTFLOAT(fTemp3);
			iRec14[1] = iRec14[0];
			for (int j0 = 3; (j0 > 0); j0 = (j0 - 1)) {
				fRec20[j0] = fRec20[(j0 - 1)];
			}
			IOTA = (IOTA + 1);
			fRec22[1] = fRec22[0];
	...
	FAUST_ADDHORIZONTALSLIDER("pressure", fHslider2, 0.5f, 0.0f, 1.0f, 0.01f);
	FAUST_ADDHORIZONTALSLIDER("velocity", fHslider1, 0.0f, 0.0f, 1.0f, 0.01f);

	#define FAUST_LIST_ACTIVES(p) \
		p(HORIZONTALSLIDER, gain, "gain", fHslider0, 0.0f, 0.0f, 1.0f, 0.01f) \
		p(HORIZONTALSLIDER, pressure, "pressure", fHslider2, 0.5f, 0.0f, 1.0f, 0.01f) \
		p(HORIZONTALSLIDER, velocity, "velocity", fHslider1, 0.0f, 0.0f, 1.0f, 0.01f) \
...
untitled::untitled() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void untitled::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }
